...
func (s *Postgres) defineRecords() error {
  ...
  CREATE INDEX IF NOT EXISTS RECORDS__IDT_IDX ON core.RECORDS(_IDT);  
	...
	CREATE OR REPLACE FUNCTION core.INDEX_BEFORE_INSERT()
  RETURNS
    TRIGGER AS $$
  BEGIN
		SELECT CAST(EXTRACT(EPOCH FROM now()) * 1000000000 AS BIGINT) INTO NEW.INDEX;
  	RETURN NEW;
  END;
  $$ LANGUAGE plpgsql;
